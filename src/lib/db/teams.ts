...
import { TEAM_CONTEXT_SELECT, mapTeamContextRow } from "./teamContext";

// The context type, the writability rule and the row mapper live in
// ./teamContext (no Supabase client bound to them) so the root layout can build
// the same object server-side. Re-exported here so existing imports keep working.
export type { TeamContext, TeamSizeBracket } from "./teamContext";
export { computeIsWritable } from "./teamContext";
import type { TeamContext, TeamSizeBracket } from "./teamContext";
